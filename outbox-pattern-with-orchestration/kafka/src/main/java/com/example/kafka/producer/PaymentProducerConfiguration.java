...
import static com.example.kafka.KafkaConstants.PAYMENT_DLQ_TEMPLATE_NAME;

@Configuration
@ConditionalOnProperty(value = "kafka.configuration.payment", havingValue = "true")
public class PaymentProducerConfiguration {
    ...
    @Bean
    public ProducerFactory<String, OrchestratorToPaymentRequest> dltFactory() {
        Map<String, Object> config = new HashMap<>();
        config.put(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG, kafkaUrl);
        return new DefaultKafkaProducerFactory<>(config, new StringSerializer(), new JsonSerializer<>());
    }

    @Bean(PAYMENT_DLQ_TEMPLATE_NAME)
    public KafkaTemplate<String, OrchestratorToPaymentRequest> retryableTopicKafkaTemplate() {
